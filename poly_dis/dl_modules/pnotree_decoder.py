...
class PianoTreeDecoder(nn.Module):
    ...
    def decoder(
        ...
        if inference:
            ...
            x_summarized = pack_padded_sequence(
                x_summarized, lengths.view(-1), batch_first=True, enforce_sorted=False
            )
            x_summarized = (
    ...
    def grid_to_pr_and_notes(self, grid, bpm=60.0, start=0.0):
        ...
        for t in range(32):
            for n in range(10):
                ...
                notes.append(
                    pretty_midi.Note(
                        100, int(pitch), start + t * alpha, start + (t + dur) * alpha
                    )
                )
        return pr, notes
    ...
    def pnotree_to_prmat(self, pnotree):
        ...
        (#, 32, 15, 6) ==> (#, 32, 128)
        """
        size = pnotree.shape[0]
        if pnotree.shape[2] == self.max_simu_note:
            pnotree = pnotree[:, :, 1:, :]
        prmat = torch.zeros((size, 32, 128))
        for batch in range(size):
            for t in range(32):
                for n in range(10):
                    note = pnotree[batch, t, n]
                    if note[0] == self.pitch_eos:
                        break
                    pitch = note[0] + self.min_pitch
                    dur = int("".join([str(_) for _ in note[1:]]), 2) + 1
                    prmat[batch, t, pitch] = min(dur, 32 - t)
        return prmat
